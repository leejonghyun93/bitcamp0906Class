 ...
 */
	...
	//③ 숫자 두 개를 매개변수의 인자로 전달받아 곱하기 메소드를 추가합시다.
	//    매개변수 : 숫자 두 개를 매개변수의 인자로 전달 받아 =>long num1,long num2 
	//    int * int => int, (long)int * int => 22억
	// long * int = long
	long multiply(long num1,int num2) {
		return num1 *num2; // 결과는 lnt
	...
	// 오버로딩의 조건 : 이름을 같다, 매개변수의 개수 또는 타입이 달라야한다!!!, 반환타입은 의미가 없다.
	float devide(float num1, float num2) {
		return num1/num2;
	}
	public static void main(String[] args) {
		// 클래스의 정의된 메소드의 사용 => 클래스를 이용해서 인스턴스 (객체)를 사용
		// 클래스의 맴버(변수,메소드)들이 메모리에 로드된다!
		...
		System.out.println("10.0/3="+cal.devide(10.0f, 3.0f));
		// 정수의 기본 타입은 => int
		// 실수의 기본 타입은 => double 
